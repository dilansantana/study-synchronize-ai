...
export const UltimateGuideGenerator: React.FC<UltimateGuideGeneratorProps> = ({ availableResources }) => {
  ...
  const extractContentFromResource = (resource: ContentItem): string => {
    let extractedInfo = '';
  ...
  const handleGenerateGuide = async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedResourcesList.length; i++) {
        ...
        const extractedInfo = extractContentFromResource(resource);
      ...
      const newGuide: ContentItem = {
        ...
        author: 'AI Assistant',
        rating: 5.0,
        content: generateGuideContent(selectedResourcesList, extractedData)
      };
